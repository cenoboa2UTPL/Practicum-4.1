@extends($this->Layouts("plantilla_home"))BaseController
...
$('#especialidad').change(function(){
 let IdEspe = $(this).val();
 mostrarDoctores(IdEspe);
});

 function mostrarDoctores(id)
 {
  let option = '';
  $.ajax({
    ...
    success:function(response){
      ...
      if(response.response.length > 0)
      {
        response.response.forEach(doctor => {
          option+=`<option value=`+doctor.id_medico+`>`+doctor.doctor+`</option>`;
        });
      }
      $('#doctor').html(option);
    }
